...
import { createWebMcpHandler, type WebMcpHandler, type WebMcpHandlerOptions } from "./web.ts";
...
export function createHttpMcpServer(options: HttpMcpServerOptions): HttpMcpServer {
  ...
  if (!isLoopbackHost(host) && !options.apiKey && options.allowInsecure !== true) {
    throw new Error(
      `Refusing to bind ${host} with no apiKey: this exposes unauthenticated tool execution to the network. ` +
      `Set apiKey, bind a loopback host (127.0.0.1), or pass allowInsecure:true to deliberately accept the exposure.`,
    );
  }

  const dnsProtection = options.enableDnsRebindingProtection ??
  ...
  const httpServer: HttpServer = createServer((req, res) => {
    void handle(req, res).catch(() => sendJsonError(res, 500, -32603, "internal error"));
  });

  async function handle(req: IncomingMessage, res: ServerResponse): Promise<void> {
    const requestPath = (req.url ?? "").split("?")[0];
    if (requestPath !== path) return sendJsonError(res, 404, -32601, `no MCP endpoint at ${requestPath}`);
    if (!webHandler) return sendJsonError(res, 503, -32000, "MCP server is starting");
    ...
    start() {
      return new Promise<void>((resolve, reject) => {
        ...
        httpServer.listen(requestedPort, host, () => {
          ...
          webHandler = createWebMcpHandler(toWebOptions(options, dnsProtection, host, boundPort));
          resolve();
        });
      });
    },
...
function toWebOptions(options: HttpMcpServerOptions, dnsProtection: boolean, host: string, port: number): WebMcpHandlerOptions {
  const { host: _host, port: _port, path: _path, allowInsecure: _allowInsecure, ...adapter } = options;
  return {
    ...adapter,
    writeToolPolicy: options.writeToolPolicy ?? "require-allowlist",
    enableDnsRebindingProtection: dnsProtection,
    ...(dnsProtection ? { allowedHosts: options.allowedHosts ?? defaultAllowedHosts(host, port) } : {}),
  };
}
...
function toWebRequest(req: IncomingMessage, host: string, port: number): Request {
  ...
  return new Request(new URL(req.url ?? "/", `http://${headers.get("host") ?? `${host}:${port}`}`), init);
}
